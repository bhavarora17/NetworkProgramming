...
int main(int argc, char *argv[])
{
    ...
    //Continue sending until all the datagrams are bytes_sent to TCPD
    while(bytes_sent<size){

        //Read from the file and store in buffer
        fread(message.body,1,sent_size,fp);

        checksum = crc32(message.body);

        memcpy((message.body+996),&checksum,sizeof(checksum));
       ...
       if((sendto(sockfd, (char *)&message, sizeof message, 0, (struct sockaddr *)&tcpd_addr, sizeof(tcpd_addr)))<1)
        ...
        if((size-bytes_sent)<sent_size){
            sent_size = size - bytes_sent;
        }
        printf("bytes_sent.. %d%\n",(bytes_sent*100)/size);
