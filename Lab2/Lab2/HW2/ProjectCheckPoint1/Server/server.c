...
int main(int argc, char *argv[])
{
	...
	while(1){
		...
		// from TCPD
		int receive_singal = recvfrom(sockfd, buffer, receive_data_size, 0, (struct sockaddr *)&serv_addr, &size_of_serv_addr);

		if(receive_singal<0){
			perror("File not received properly");
			exit(1);
		}

		if(receive_singal<4){
			printf("Less than 4 Bytes received\n");
			continue;
		}

		printf("Received : %d\n", receive_singal);
		
		char data[receive_singal-4];

		memcpy(data,buffer,(receive_singal-4));

		memcpy(&checksum,(buffer+receive_singal-4),4);

		if(checksum == crc32(data)){
			printf("Checksum Correct for packet number: %d\n",count++);
		}
		else {
			printf("Checksum Wrong for packed number (Garbled Packet)%d\n",count++);
		}

		printf("Checksum  is and %u CRC is %u\n", checksum, crc32(data));
